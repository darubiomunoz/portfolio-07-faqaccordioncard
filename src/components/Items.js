        ...
        return (
          <li key={data.id} className="faq__item" tabIndex="-1">
            <div className="faq__container">
              ...
              <img
                id={data.id}
                className={`faq__icon ${id.includes(data.id) ? "rotate" : ""}`}
                src={arrow}
                tabIndex="0"
                aria-label={
                  id.includes(data.id)
                    ? "Press enter to hide answer"
                    : "Press enter to show answer"
                }
                onClick={handleClick}
                onKeyPress={handleClick}
                aria-live="polite"
                alt="Press enter to show answer"
              />
            </div>
            <p
              className={`faq__answer ${
                id.includes(data.id) ? "" : "invisible"
              }`}
              tabIndex={id.includes(data.id) ? "0" : "-1"}
              aria-live="off"
            >
              {data.answer}
            </p>
